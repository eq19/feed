...
RUN [ -d /mnt/disks/deeplearning ] && curl -H "Authorization: Bearer $(/mnt/disks/deeplearning/usr/bin/gcloud auth application-default print-access-token)" \
  "https://secretmanager.googleapis.com/v1/projects/feedmapping/secrets/freqtrade-config/versions/latest:access" | \
  jq -r '.payload.data' | base64 --decode > /freqtrade/config.json && chown ftuser:ftuser /freqtrade/config.json
