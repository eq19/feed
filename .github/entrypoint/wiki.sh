...
rm -rf ${TEMP_FOLDER}/README.md

mv -f ${TEMP_FOLDER}/wikidir/README.md ${TEMP_FOLDER}/README.md
cp -R ${TEMP_FOLDER}/wikidir/* $1

rm -rf ${TEMP_FOLDER}/spin.txt && touch ${TEMP_FOLDER}/spin.txt
find $1 -type f -name 'spin_*.txt' | sort -n -t _ -k 2  | while ((i++)); IFS= read -r f; do sort.sh $f $i; done

rm -rf ${TEMP_FOLDER}/Sidebar.md && cp $1/_Sidebar.md ${TEMP_FOLDER}/Sidebar.md
sed -i 's/0. \[\[//g' ${TEMP_FOLDER}/Sidebar.md && sed -i 's/\]\]//g' ${TEMP_FOLDER}/Sidebar.md

find $1 -iname '*.md' -print0 | sort -zn | xargs -0 -I '{}' front.sh '{}'

echo -e "\n$hr\nSPIN\n$hr"
cat ${TEMP_FOLDER}/spin.txt
