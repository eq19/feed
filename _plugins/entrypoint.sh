...
set_target() {
  ...
  if [[ ! "${array_str},," =~ ",,$1,," ]]; then TARGET_REPOSITORY=$1/${array[0]}
  elif [[ ${array[-1]} == $1 ]]; then TARGET_REPOSITORY=${OWNER}/${OWNER}.github.io
  else
    for i in 0 1 2 3 4 5; do
      [[ ${array[$i]} == $1 && $i -lt 5 ]] && TARGET_REPOSITORY=$1/${array[$i+1]}
    done
  fi
}
...
[[ ${GITHUB_REPOSITORY} == *"github.io"* ]] && set_owner ${GITHUB_REPOSITORY_OWNER}
